...
class FiveStageSimulator final : public Simulator {
  ...
  bool mispredicted_branch_ = false;
  bool should_recover_branch_ = false;
  int64_t branch_next_pc_ = 0;
 ...
 public:
  explicit FiveStageSimulator(const Options& opts) : Simulator(opts),enable_predict(opts.enable_predict){
