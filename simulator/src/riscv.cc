...
int64_t HandleSystemCall(bool* exit_ctrl, int64_t type /* REG_A7 */,
  ...
  *exit_ctrl = false;
  switch (type) {
    ...
    default:
      throw std::runtime_error(fmt::format("Unknown syscall type {}\n", type));
  }
  return arg1;
}
  ...
  switch (opcode) {
      ...
      switch (funct3) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown Funct3 field {:#x}\n", funct3));
      }
    ...
    case OP_BRANCH:
      ...
      switch (funct3) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown funct3 {:#x} at OP_BRANCH\n", funct3));
      }
      inst_name = INSTNAME[inst_type];
      op1_str = REGNAME[rs1];
      op2_str = REGNAME[rs2];
    ...
    case OP_STORE:
      ...
      switch (funct3) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown funct3 {:#x} for OP_STORE\n", funct3));
      }
      inst_name = INSTNAME[inst_type];
      op1_str = REGNAME[rs1];
      op2_str = REGNAME[rs2];
    ...
    case OP_LOAD:
      ...
      switch (funct3) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown funct3 {:#x} for OP_LOAD\n", funct3));
      }
      inst_name = INSTNAME[inst_type];
      op1_str = REGNAME[rs1];
      op2_str = std::to_string(op2);
    ...
    case OP_IMM32:
      ...
      switch (funct3) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown funct3 {:#x} for OP_ADDIW\n", funct3));
      }
      inst_name = INSTNAME[inst_type];
      op1_str = REGNAME[rs1];
      op2_str = std::to_string(op2);
      ...
      switch (funct3) {
        case 0x0:
          if (temp == 0x0) {
            ...
            throw std::runtime_error(
                fmt::format("Unknown 32bit funct7 {:#x}\n", temp));
          }
          break;
        case 0x1:
          if (temp == 0x0) {
        ...
        default:
          throw std::runtime_error(
              fmt::format("Unknown 32bit funct3 {:#x}\n", funct3));
      }
      inst_name = INSTNAME[inst_type];
      op1_str = REGNAME[rs1];
      op2_str = REGNAME[rs2];
...
void ExecuteInst(PipeOp* op, bool* exit_ctrl, MemoryManager* mem) {
  ...
  // // handle the branch change 这里要正常进行地址的计算 通过这一步比较预测的正确性
  ...
  switch (inst_type) {
    ...
    default:
      throw std::runtime_error(fmt::format("Unknown instruction type: {}\n",
