...
void FiveStageSimulator::Decode() {
  ...
  try {
    DecodeInst(op, regs_);
    if(enable_delay){
    if(IsMulType(op->inst_type)){
      op->remaining_cycles=5;
    }
  }
...

  {
    ...
    oss << "0x" << std::setw(8) << std::setfill('0') << std::hex << op->pc
        << std::dec << ": " << op->inst_str << "\n";
    history_.inst_record.emplace_back(oss.str());
  }
  ...
  if (execute_op_ != nullptr) {
    if (verbose_) {
      std::cout << "Decoded instruction 0x" << std::hex << op->inst
                << " at address 0x" << op->pc << " as " << op->inst_str << "\n"
                << std::dec;
    }
    return;
  }
...
void FiveStageSimulator::Execute() {
  ...
  if(enable_delay){
      if (op->remaining_cycles > 1) {
        op->remaining_cycles--;
        return;
      }
    }
  if (verbose_) {
    std::cout << "Execute instruction 0x" << std::hex << op->inst
              << " at address 0x" << op->pc << " as " << op->inst_str << "\n"
              << std::dec;
  }
  history_.inst_count++;
...
void FiveStageSimulator::MemoryAccess() {
  ...
  if (verbose_) {
    std::cout << "MemoryAccess instruction 0x" << std::hex << op->inst
              << " at address 0x" << op->pc << " as " << op->inst_str << "\n"
              << std::dec;
  }
...
void FiveStageSimulator::WriteBack() {
  ...
  if (verbose_) {
    std::cout << "WriteBack instruction 0x" << std::hex << op->inst
              << " at address 0x" << op->pc << " as " << op->inst_str << "\n"
              << std::dec;
  }
...
std::string FiveStageSimulator::GetRegInfoStr() const {
  ...
  str += std::string("PC: 0x") + [&]() {
    std::ostringstream oss;
    oss << std::hex << pc_ << std::dec;
    return oss.str();
  }() + "\n";
  for (uint32_t i = 0; i < 32; ++i) {
    std::ostringstream oss;
    oss << std::hex << std::setw(16) << std::setfill('0') << regs_[i]
        << std::dec;
    str += std::string(REGNAME[i]) + ": 0x" + oss.str() + "(" +
           std::to_string(regs_[i]) + ") ";
    if (i % 4 == 3) {
      str += "\n";
    }
  }
