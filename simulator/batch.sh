...
build)
  ...
  if [[ "$target" == "with-syscall" ]] || [[ "$target" == "all" ]]; then
    ...
    for test in helloworld test_arithmetic test_syscall test_branch quicksort matrixmulti ackermann test_rem; do
  ...
  if [[ "$target" == "without-syscall" ]] || [[ "$target" == "all" ]]; then
    ...
    for test in add double-float mul-div n! qsort simple-function; do
      $CC -march=rv64im -mabi=lp64 $SRC_WITHOUT_SYSCALL/${test}.c -o $BUILD_DIR/without-syscall/${test}.riscv
