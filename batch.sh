...
build)
  ...
  if [[ "$target" == "with-syscall" ]] || [[ "$target" == "all" ]]; then
    ...
    for test in helloworld test_arithmetic test_syscall test_branch quicksort matrixmulti ackermann test_find_min; do
...
run)
    ...
    for test in $BUILD_DIR/with-syscall/*.riscv; do
      if [[ -f "$test" ]]; then
        ...
        if [[ -f "$input_file" ]]; then
          $SIMULATOR -i "$test" -k "$KERNEL_IMG" <"$input_file"
        else
          $SIMULATOR -i "$test" -k "$KERNEL_IMG"
        fi
      fi
    done
  fi
  ...
  if [[ "$target" == "without-syscall" ]] || [[ "$target" == "all" ]]; then
    for test in $BUILD_DIR/without-syscall/*.riscv; do
      [[ -f "$test" ]] && echo "Running $(basename $test)..." && $SIMULATOR -i "$test" -k "$KERNEL_IMG"
    done
  fi
  ;;
