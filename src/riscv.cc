    ...
    case OP_SYSTEM:
      if (funct3 == 0x0 && funct7 == 0x000) {
        ...
        if(op2==7) {
          ...
          csrs[CSR_MCAUSE]=11;//系统调用
          csrs[CSR_MEPC]=op->pc+4;
          // printf("csrs[CSR_MEPC]={%x}\n",csrs[CSR_MEPC]);
          dest_reg = 0;
        }
        else{
          op1 = regs[REG_A0];
      ...
      }else if(funct3 == 0x0 && funct7 == 0x018){
        op1=csrs[CSR_MEPC];
        // printf("MRET at PC {%x}, jump to {%x}\n",op->pc,op1);
          // printf("csrs[CSR_MTVEC]={%x}\n",csrs[CSR_MTVEC]);
        inst_type = MRET;
      }else if(funct3 == 0x1){
        op1 = csrs[csr];
        csrs[csr]=regs[rs1];
...
void ExecuteInst(PipeOp* op, bool* exit_ctrl, const Memory* mem, Csrs& csrs) {
  ...
  switch (inst_type) {
    ...
    case ECALL: {
      ...
      if(op2==7){
        // jump_pc = 0x00020000;
        jump_pc=csrs[CSR_MTVEC];
        branch = true;
        // printf("ECALL-Jump to {%x}\n",jump_pc);
      }
      else {
